 ...
 * 2. Offline (buildProto): fully client-side, no network calls
...
async function main(): Promise<void> {
  ...
  await wallet.signTransaction(assistedTx)
  const hash1 = await wallet.broadcastTransaction(assistedTx)
  ...
  await wallet.signTransaction(offlineTx)
  const hash2 = await wallet.broadcastTransaction(offlineTx)
