...
vi.mock('@klever/connect', () => {
  ...
  class FakeBuilder {
    ...
    setITOPrices(req: Record<string, unknown>): this {
      this._c = { contractType: 16, ...req }
...
describe('ito-set-prices example', () => {
  beforeEach(() => {
    captured.contracts.length = 0
    process.env['PRIVATE_KEY'] = 'a'.repeat(64)
    process.env['KDA_ID'] = 'MYTKN-AB12'
    process.env['PACK_INFO_JSON'] =
      '{"KLV":{"packs":[{"amount":"1","price":"2000000"}]}}'
    process.env['DRY_RUN'] = 'true'
  })

  it('forwards the parsed packInfo', async () => {
    vi.resetModules()
    await import('./index.js')
    await new Promise((r) => setTimeout(r, 10))
    const c = captured.contracts[0]!
    expect(c['contractType']).toBe(16)
    const packs = (c['packInfo'] as Record<string, { packs: { price: string }[] }>)['KLV']!.packs
    expect(packs[0]!.price).toBe('2000000')
  })
})
