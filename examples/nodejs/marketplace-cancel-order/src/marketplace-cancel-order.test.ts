...
vi.mock('@klever/connect', () => {
  ...
  class FakeBuilder {
    ...
    cancelMarketOrder(req: Record<string, unknown>): this {
      this._c = { contractType: 19, ...req }
...
describe('marketplace-cancel-order example', () => {
  beforeEach(() => {
    captured.contracts.length = 0
    process.env['PRIVATE_KEY'] = 'a'.repeat(64)
    process.env['ORDER_ID'] = 'd'.repeat(64)
    process.env['DRY_RUN'] = 'true'
  })

  it('forwards orderId only', async () => {
    vi.resetModules()
    await import('./index.js')
    await new Promise((r) => setTimeout(r, 10))
    const c = captured.contracts[0]!
    expect(c['contractType']).toBe(19)
    expect(c['orderId']).toBe('d'.repeat(64))
